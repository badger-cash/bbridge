...
**A reference implementation of this spec now exists** in `packages/contracts` (`BridgeLock.sol` + `contracts/lib/`), with 22 passing tests including a full, cryptographically real end-to-end withdrawal (`test/release.test.js`) and cross-validation of the Merkle/PoW logic against an actual mined XEC block (`test/lib.realblock.test.js`) — see that package's README for what's been tested and how. Writing it settled several of the open questions below with concrete (if still provisional) answers, noted inline where that happened. This document wasn't rewritten around those choices — it still describes the target design; the implementation notes mark where reality and this draft now diverge or agree.
...
**Implementation note, since corrected:** an earlier draft of this document (and of `BridgeLock.sol`) added an `xecTokenId` parameter here, checked against the burn transaction's self-reported `token_id` field in `release()`. That check was removed. It didn't provide real security: a `token_id` field in a transaction's OP_RETURN is a claim, not proof — actual SLP validity requires indexer-style graph traversal back to a valid GENESIS, which no single transaction's bytes can establish and which this contract was never actually verifying, just string-matching against a stored constant. The real guarantee that a burn represents the correct token is the Authorizer's decision to co-sign the postage stamp input at all (§6) — presumably backed by real SLP indexing on the Authorizer's side. Re-checking `token_id` on top of that added gas cost and a false sense of independent verification without adding real security. `assetId` is not affected by this and is still checked (§6) — it's a different kind of check, a trivial self-referential domain separator (`assetId == address(this)`) that needs no external trust, unlike `token_id`.
...
- **The vault outpoint's byte order — now concrete.** `utxoTxid` is internal byte order (matching `EcashTx.sol`'s convention elsewhere in this contract) and `utxoIndex` is little-endian (§2.4, §4) — chosen to match a BIP143 preimage's own embedded outpoint field exactly, so the covenant can compare them with no conversion. Still open: whether the eCash self-mint covenant redesign (`overview.md` §9) actually ends up expecting this exact encoding once it's implemented for real.
...
- **New question, found during implementation: is `tx.signature()`'s Schnorr-by-default behavior in `@hansekontor/checkout-components` a problem anywhere else in `packages/sdk`?** `test/release.test.js` had to bypass it (signing manually, matching `lib/oracle.js`'s existing pattern) to produce a signature `EcashTx.parseDER` — and standard P2PKH `OP_CHECKSIG` — would actually accept. `lib/oracle.js` already avoids the wrapper everywhere it matters, so this is likely already a non-issue in `packages/sdk` itself, but worth confirming deliberately rather than assuming, since it's an easy trap for any *new* SDK code (e.g. the eventual burn/postage transaction builder, `overview.md` §9) that reaches for the convenience wrapper instead.
