...
5. **A confirmation signature must not be usable before its deposit is irreversibly confirmed — closed operationally, not by changing `message`.** Found in the 2026-07 review: `confirmDeposit`'s signature is valid the instant the Authorizer produces it, independent of whether the transaction carrying it ever mines — nothing in `message` (not even after the §2.3 `depositId` fix) makes its validity conditional on the Ethereum call succeeding. If the vault UTXO it references is already spendable on XEC at that point, a depositor who reads the signature out of a public mempool before `confirmDeposit()` mines can front-run it with `refund()` (§4 below), collect their full refund, and still complete an unbacked mint with the untouched signature — the pending `confirmDeposit()` call then simply reverts (`AlreadyRefunded`) with no on-chain trace of having been attempted. Unlike the other corrections in this section, this one has no fix expressible as a `message` change or an added Solidity check, because the gap isn't in what's signed or what the contract verifies — both are already correct. It's in the operational relationship between two independent facts that this design deliberately keeps unlinked: "this signature is valid" (a pure function of `message` and the Authorizer's key, true forever once produced) and "the vault UTXO it references is real and spendable on XEC" (a fact about XEC chain state, which nothing in this contract or the eCash covenant can make conditional on Ethereum state, by design — `overview.md` §7, "Independent verifiability"). The fix is a hard requirement on the Authorizer service's implementation: **vault UTXO quarantine** (`docs/SPEC.md` §III.7) — the Authorizer must construct the vault-funding transaction and know its txid before ever signing a confirmation that references it, but must not broadcast that transaction to XEC until it has independently observed the corresponding `confirmDeposit()` reach Ethereum finality. This makes the two facts above effectively-simultaneous in the only case that matters (a signature is never independently useful before its referenced coin exists), even though the contract itself has no way to enforce or verify that the Authorizer actually does this.
...
| `refundDelay` | minimum Ethereum blocks that must elapse between a `requestRefund()` call and `refund()` itself succeeding (§4, §2.5) |

`minDifficultyTarget` and `refundDelay` in particular need real answers before deployment, not placeholders — see §8.
...
### `requestRefund(bytes32 depositId) external` / `cancelRefundRequest(bytes32 depositId) external`

Added 2026-07 as a defense-in-depth layer on top of vault UTXO quarantine (§2.5), not a replacement for it. `refund()` is no longer directly callable the instant a deposit is unconfirmed; it now additionally requires this two-step handshake:

- `requestRefund(depositId)`: `require(msg.sender == depositor && !confirmed && !refunded)`, then record `refundRequestedAt[depositId] = block.number` and emit `RefundRequested(depositId, block.number)`. Moves no funds. Re-callable while a request is already live — a second call simply restarts the cooldown from the new block, it does not revert.
- `cancelRefundRequest(depositId)`: `require(msg.sender == depositor)`, `require(refundRequestedAt[depositId] != 0)`, then delete the entry and emit `RefundRequestCancelled(depositId)`. Exists so a depositor who changes their mind isn't stuck perpetually signalling refund intent with no way to retract it; a later `requestRefund()` call pays the full `refundDelay` again rather than resuming progress.

The point of this handshake is purely to give the Authorizer's own monitoring an observable, bounded warning window it never had under a single-step refund (whose front-run was instantaneous and unannounced) — see the note after `getAuthorization` below for what this mechanism does and does not actually guarantee.
...
4. `require(refundRequestedAt[depositId] != 0, "RefundNotRequested")`, then `require(block.number >= refundRequestedAt[depositId] + refundDelay, "RefundDelayNotElapsed")` — the requestRefund()/refundDelay handshake above.
5. Mark refunded, transfer the *full* original locked amount (not net of fee — the doc's key representations describe the refund path as returning "the full locked amount") back to `msg.sender`.
...
**A note this table can't fully capture:** invariant 7's enforcement above assumes the referenced vault UTXO isn't spendable on XEC until its confirmation has actually landed on Ethereum. Nothing in this table's "where enforced here" column is Solidity — that assumption is enforced by the Authorizer's own operational discipline (vault UTXO quarantine, §2.5, `docs/SPEC.md` §III.7), external to both this contract and the eCash covenant. It's listed here rather than omitted because it's genuinely load-bearing for invariant 7, not because this contract does anything to enforce it. `refund()`'s `requestRefund()`/`refundDelay` gate (§4) is the one piece of on-chain code that does exist for this problem, but it's additional friction against the race, not enforcement of the assumption above — see the note after `getAuthorization` in §4 for why it can't be.
...
- **`refundDelay`'s actual value — implemented, real value still open.** The `requestRefund()`/`cancelRefundRequest()`/`refundDelay` mechanism (§4, 2026-07 review) is a real, tested defense-in-depth layer on top of vault UTXO quarantine, not a placeholder awaiting design. What remains open is the deployment-time value itself: the test suite uses 20 blocks as a representative placeholder, not a measured one — a real deployment should set this once the Authorizer service's actual sign-to-mined latency (including under gas-spike conditions) is characterized.
...
- **Resolved as a documented requirement, plus an on-chain defense-in-depth layer: the confirm/refund signature race (§2.5).** A depositor front-running a pending `confirmDeposit()` with `refund()` and still using the extracted, unconditionally-valid signature to complete an unbacked mint on XEC. No `message` change or added Solidity check can close this outright — the actual, structural fix is a hard, documented requirement on the Authorizer service's implementation (vault UTXO quarantine, `docs/SPEC.md` §III.7): never broadcast a vault-funding transaction until the confirmation it backs has reached Ethereum finality. This is the standard mitigation for this exact class of gap in self-mint-protocol-style bridges. Still open: the Authorizer service itself isn't built yet, so quarantine remains a specified requirement rather than a verified-in-practice one. Layered on top of it, `requestRefund()`/`refundDelay` (§4, 2026-07 review) *is* now implemented and tested — a two-step cooldown before `refund()` becomes callable, giving the Authorizer's monitor advance, observable warning of refund intent it never had under a single-step refund. It narrows the practical window for the race but is a best-effort mitigation, not a replacement for quarantine: it only helps if the Authorizer actually watches for it, and does nothing for a signature already extracted before the request was ever made.
