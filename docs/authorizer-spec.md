...
3. **Validate input 0.** Signed `SIGHASH_ALL | SIGHASH_FORKID | SIGHASH_ANYONECANPAY`, standard P2PKH `<signature> <pubkey>` form, and `hash160(pubkey)` equal to the OP_RETURN's `recipientHash160` (Section IV.4).
...
- **It is a floor on the burn declaration, not on any single UTXO.** Burn quantity sums across the transaction's token inputs, so a holder whose balance is spread across dust UTXOs consolidates them into one burn that clears the floor. The rule costs them a consolidation, not their money.
