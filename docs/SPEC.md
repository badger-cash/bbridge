...
### Status: draft — architecture and message formats below are implemented and tested (`packages/sdk`: 35 passing cases; `packages/contracts`: 79 passing cases, including a full deposit-to-release round trip spanning both chains in a single test); several deployment parameters and byte-level details remain reserved for future specification (Appendix A)
...
**Originally specified as an Authorizer-service-only requirement; now also enforced on-chain for the honest-key threat model (revised 2026-07 review, round 4).** Section IV.3 step 7's postage-outpoint tracking (`stampUtxoConsumedBy`) stops a *single* stamp from backing more than one release — it says nothing about whether the *same underlying burn declaration* (identical input 0, identical OP_RETURN) could legitimately be stamped a second time under a *different, fresh* stamp coin. An on-chain single-use mapping keyed on input 0's outpoint was originally considered and rejected for this purpose, on the reasoning that nothing on-chain can distinguish a genuine burn coin's outpoint from one an attacker with a compromised key invents outright (see the note on key compromise below) — that reasoning is still correct as far as it goes. A follow-up audit pass found it didn't go far enough: because input 0's `SIGHASH_ANYONECANPAY` signature is valid glued to any co-input, and `release()`'s header check never requires real chain-tip inclusion, an *honest* Authorizer key producing two distinct, both-genuine stamps for the same declaration (an ordinary service race or retry) is sufficient **on its own** for a second full release — no forged signature or invented outpoint needed, so the "attacker can just fabricate a new outpoint" argument doesn't apply to this narrower case. `BridgeLock.sol` now enforces this on-chain via `burnUtxoConsumedBy`, keyed on input 0's own outpoint (mirroring `utxoConsumedBy` on the deposit side) — see that mapping's own doc comment and `release()`'s "HONEST-KEY DOUBLE-STAMP" doc comment.

**The requirement (defense-in-depth, on top of the on-chain check above).** The Authorizer's postage service, before ever cosigning a stamp for a given burn declaration, **should still** check whether it has already cosigned a stamp for a declaration with the same input 0 outpoint and the same OP_RETURN content, and refuse to sign a second one — catching the problem before it ever reaches the chain, rather than relying solely on `release()`'s revert. This is a deduplication check the service is already positioned to make — it necessarily inspects input 0's outpoint and the OP_RETURN before deciding whether a burn is legitimate to stamp at all (the "business responsibility" described in Section IV.1).
...
- **Gas cost of withdrawal processing.** Measured. `release()` costs about 448,000 gas — it parses a burn transaction, verifies two eCash signatures, walks a Merkle branch and validates a block header, and `BridgeLock.sol` needs `viaIR: true` to compile at all. `confirmDeposit()` is about 186,000. Both are asserted against ceilings in `packages/contracts/test/gas.test.js` (500,000 and 220,000), so a regression fails the suite rather than going unnoticed; `REPORT_GAS=true npx hardhat test` prints the full table. Gas is deterministic given bytecode and calldata, so these are exact rather than indicative — what remains a deployment question is the gas *price* a user faces, which is a market condition rather than a property of this contract.
