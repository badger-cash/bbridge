...
| `getAuthorization(bytes32 depositId) external view returns (bool confirmed, bytes20 xecRecipient, uint64 xecAmount)` | Public, unauthenticated read of a deposit's authorization content. `xecAmount` is the XEC-side (post-conversion) quantity actually signed, recomputed from stored state rather than stored separately. The signature and the vault outpoint are published in the `DepositConfirmed` log rather than stored — the eCash covenant is their only consumer and it verifies them directly, so storing them cost four slots per deposit for a lookup nothing performs. |
