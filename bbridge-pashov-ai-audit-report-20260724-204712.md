...
|                                  |                                                        |
| -------------------------------- | ------------------------------------------------------ |
| **Mode**                         | Consolidated report across five independent review rounds — see [Audit History](#audit-history) |
| **Files reviewed**               | `BridgeLock.sol` · `Difficulty.sol`<br>`EcashTx.sol` · `MerkleProof.sol` · `Sighash.sol` |
| **Confidence threshold (1-100)** | 75 (rounds 3–5) / 80 (rounds 1–2) |

This document replaces `bbridge-pashov-ai-audit-report-20260723-184554.md` (round 1 only). It consolidates all five review rounds performed against this codebase to date into a single record: what was found, what was fixed, what was deliberately accepted as a documented trust boundary instead of fixed, and what remains open. Every finding below carries a **Status** and, for resolved items, the commit that closed it — this is the traceability layer the individual round reports didn't provide on their own.

**Test suite status as of this report:** `packages/contracts`: 77/77 passing · `packages/sdk`: 35/35 passing. (Round 5 was an audit-only re-verification pass; it produced no code changes, so the test count is unchanged from round 4.)

**Note on round-5 model routing:** round 5 was requested with the Claude Fable 5 model for its 12 agents, but per the operator, Fable security-work requests are flagged and served by Claude Opus 4.8, so the agents effectively ran on Opus 4.8. This attribution reflects the operator's account of backend routing; the orchestrator does not have independent visibility into the resolved subagent model.
...
| 5 | 2026-07-25 | 12-agent parallel review (solidity-auditor skill, fresh scan; agents ran on Opus 4.8 per routing) | 0 findings, 13 leads | — | — | 13 leads (3 net-new, all low-severity; 10 re-corroborate rounds 3–4) |
...
## Round 5 — Clean re-verification pass (2026-07-25)

Fresh 12-agent parallel scan (same solidity-auditor skill and specialty roster as rounds 3–4), run the morning after round 4's four fixes landed. The run was requested with the Claude Fable 5 model; per the operator, Fable security-work requests are flagged and served by Claude Opus 4.8, so the 12 agents effectively executed on Opus 4.8 (see the model-routing note at the top of this report — the attribution reflects the operator's account of backend routing, not an observation the orchestrator could make directly).

**Outcome: zero findings.** No agent produced a FINDING with an unguarded, in-scope, attacker-reachable exploit path. This is the intended result of a re-verification pass: it confirms that the 15 fixes accumulated across rounds 1–4 hold, and that round 4's four new code paths did not introduce fresh exploitable issues. Specific independent re-verifications worth recording:

- **`burnUtxoConsumedBy`** (round 4 Finding 1) — confirmed holding by agents 2, 4, 8, 9, 12: withdrawal replay is blocked by outpoint-keyed tracking, invariant under any signature re-encoding.
- **`chainId` in the BURN OP_RETURN** (round 4 Finding 2) — confirmed by agents 2, 7, 9, 12: `burnChainId != bytes32(chainId)` reverts a same-address cross-chain replay.
- **Low-S canonicalization in `confirmDeposit()`** (round 4 Finding 3) — confirmed by agents 2, 3, 7, 9, 12: the `s > _SECP256K1_N_DIV_2` check rejects the malleated twin before `ecrecover`.
- **`getAuthorization()` `UnknownDeposit` check** (round 4 Finding 4) — agents 8 and 10 specifically diffed `getAuthorization()` against `confirmDeposit()` and proved the `xecAmount` arithmetic byte-identical and drift-free; the added existence check introduced no view/write asymmetry.

The collateral-conservation invariant (releases + refunds can never exceed genuinely-locked backing) was re-derived with concrete numbers in both decimal-scale directions by agents 1, 3, 5, 7, 8, 10, and 11 — all confirmed every conversion floors toward the vault, with `collectedDust`/`pendingXecDust` as pure counters that never gate a payout.

Round 5 raised 13 leads. Ten re-corroborate rounds 3–4's already-open leads (the deployer-parameter DoS family, fee-rounding self-harm, locked protocol revenue, the confirm/refund cross-chain race, the weak-header/single-factor-trust tradeoff, and the out-of-scope `EcashTx.decompress` / `parseDER` library gaps). Three are net-new and low-severity — folded into the Open Leads section below and marked `[Round 5]`.

---

## Open Leads (rounds 3–5 combined)

_Vulnerability trails with concrete code smells where the full exploit path could not be completed. Not scored; not yet acted on as of this report. Leads independently re-corroborated by later rounds are noted inline; round-specific leads are marked as such. Round 5 re-corroborated the decimals-gap, `EcashTx.decompress`, `EcashTx.parseDER`, locked-revenue, and weak-header leads below (noted inline) and added three net-new low-severity leads at the end._

- **Unbounded deployer decimals gap** — `BridgeLock.constructor` / `release` — `tokenDecimals_` is an unrestricted `uint8` (0-255) vs. genesis-parsed `decimals_` (0-9); `scale_ = 10 ** decimalsGap` can revert deployment outright for large gaps, or make round-2-style fee truncation near-100% instead of the ~33-50% shown in earlier math. **Round 5 (agents 1, 3, 5, 12) sharpened the DoS variant:** beyond the construction-time overflow, a large deployer-chosen `feeAmount` against a large scale can make `feeAmountXec = feeAmount * scale` exceed `type(uint64).max`, so `release()`'s `burnQuantity <= feeAmountXec` gate rejects every possible `uint64` burn and permanently bricks all withdrawals while deposits still lock funds — no constructor bound couples `feeAmountXec` to the uint64 mint ceiling. All deployer-misconfiguration at immutable construction time; no attacker path.
...
- **`EcashTx.decompress` never verifies the recovered point is actually on the curve** — `EcashTx.decompress` — no `mulmod(y,y,P) == x³+7 mod P` check after MODEXP. **Re-corroborated in rounds 4 and 5** (round 4: agents 7 + 12; round 5: agents 2, 4, 6, 9, 12 — nine independent agents across two rounds now): all traced the only two consumers (`pubkeyHash160`/`ethRecipient` derivation and `verifyAgainstPubkey`'s address comparison against a real `ecrecover` result) and confirmed exploitation would require either a hash-preimage break or an off-curve `x` whose fabricated point happens to derive an address matching a real recovered key — both infeasible under current cryptographic assumptions. A robustness gap, not a demonstrated bypass. Suggested defensive fix (round 5): `require(mulmod(y, y, P) == rhs)` after deriving `y`.
- **`EcashTx.parseDER` accepts non-minimal-length / non-canonical encodings** — `EcashTx.parseDER` — `rLen`/`sLen` taken from a caller byte with no bound tied to 32, no strict-DER/low-S enforcement, total-length byte unchecked; widens the malleability surface, but confirmed not to bypass the outpoint-keyed replay guards since outpoints are unaffected by re-encoding. **Re-corroborated in round 5** (agents 4, 6, 7, 9). Note: round 4 Finding 3 closes the *high-S* malleability vector on `confirmDeposit()`'s stored signature specifically; this lead is about DER-encoding non-canonicality more broadly in the burn/stamp verification path, which remains open.
...
- **[Round 5] `xecTokenId` byte-order may not match the eCash covenant's expected `token_id` order** — `BridgeLock.constructor` / `_buildMintTxOutputs` / `_parseBurnOpReturn` — found by agent 5 (invariant). `xecTokenId = sha256(sha256(rawGenesisTx_))` is the *internal* (sha256d) txid byte order; SLP OP_RETURN `token_id` fields are conventionally pushed in *displayed* (reversed) order. The contract is internally self-consistent — it builds MINT OP_RETURNs and parses BURN OP_RETURNs both using `xecTokenId` verbatim — so no on-chain check can be tricked. But if the out-of-scope eCash self-mint covenant / SLP validators expect the reversed order, every mint this contract authorizes and every burn it accepts would carry a `token_id` no real validator recognizes: a total-liveness failure, not a theft path. Unverifiable in-scope (covenant not in the bundle). **Worth a cheap cross-check against `packages/sdk`'s covenant `token_id` encoding**, since being wrong would break the whole system while being trivial to confirm.
- **[Round 5] Rebasing `token` silently breaks refund/release solvency** — `BridgeLock.deposit` / `refund` — found by agent 12 (flow-gap). The `deposit()` balance-delta accounting is scoped by its own doc comment to fee-on-transfer tokens; it does not address a *negative-rebasing* `token`, for which the contract's aggregate balance can fall below the sum of recorded `netAmount`s, breaking the invariant-3 guarantee that "refund returns the full original locked amount" for the last claimants. `token` is deployer-chosen and immutable, so this is a deployment hazard rather than an attacker-reachable path; worth an explicit "non-rebasing tokens only" constraint in the deployment docs.
- **[Round 5] `MerkleProof.deriveRoot` reject-sentinel (`bytes32(0)`) can collide with an attacker-chosen root** — `MerkleProof.deriveRoot` / `verify` (out-of-scope lib) — found by agent 6 (periphery). `deriveRoot` uses `bytes32(0)` as its CVE-2012-2459 "malleability detected, force mismatch" sentinel, and `verify` compares the derived value against a caller-supplied `root`. An attacker who sets the header's merkleRoot field to all-zeros and crafts a branch that trips the guard gets `deriveRoot` returning `bytes32(0) == root`, so `verify` returns true. **Moot in `BridgeLock` today** — the self-mined-header design already makes the merkle proof a non-barrier (the Authorizer stamp signature is the real gate), and an attacker has a strictly easier bypass (set merkleRoot = burnTxid, empty branch) — but a live bug if `MerkleProof` is ever reused in a context where `root` is trusted or non-attacker-controlled. Suggested fix: signal malleability via a separate boolean/revert rather than an in-band `bytes32(0)`, or have `verify` reject a zero derived root.
